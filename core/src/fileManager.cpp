...
bool FileManager::loadConfig(std::filesystem::path path,
    ...
    if (!std::getline(file, line)) {
        ...
        if (!(size >> height >> width)) {
            std::cerr
                << "Erreur: le format n'est pas respecté -> <height> <width>"
                << std::endl;
            return false;
    ...
    for (int y = 0; y < height; y++) {
        if (!std::getline(file, line)) {
            std::cerr << "Erreur: colonne manquante -> y = " << y << std::endl;
            return false;
        };
        ...
        for (int x = 0; x < width; x++) {
            ...
            if (!(lineContent >> value)) {
                ...
                std::cerr << "Erreur: la valeur doit être 0 ou 1 -> (y=" << y
                          << ", x=" << x << ")" << value << std::endl;
                return false;
            }

            gameGrid->setCell(x, y, static_cast<CellState>(value));
        };
    ...
    outputPath =
        "../data/outputs/" + this->getConfigName().stem().string() + "_out.txt";
    this->clearOutputFileIfContent();
    return true;
}

bool FileManager::makeConfig(std::filesystem::path path, int height, int width,
                             std::unique_ptr<Grid> &gameGrid,
                             bool isEmptyConfig) {
    this->configPath = path;
    ...
    gameGrid = std::make_unique<Grid>(height, width);
    file << gameGrid->getHeight() << " " << gameGrid->getWidth() << std::endl;
...
std::filesystem::path FileManager::getConfigName() const {
...
void FileManager::saveOutput(std::unique_ptr<Grid> &grid) {
    ...
    for (int y = 0; y < grid->getHeight(); y++) {
        for (int x = 0; x < grid->getWidth(); x++) {
            file << (grid->getCell(x, y)->getState() == CellState::ALIVE ? "1"
                                                                         : "0")
                 << " ";
        }
        file << std::endl;
...
bool FileManager::saveConfig(std::unique_ptr<Grid> &grid) {
    std::ofstream file(this->configPath, std::ios::trunc);
    ...
    for (int y = 0; y < grid->getHeight(); y++) {
        for (int x = 0; x < grid->getWidth(); x++) {
            file << (static_cast<int>(grid->getCell(x, y)->getState()) ? "1"
                                                                       : "0")
                 << " ";
        }
        file << std::endl;
...
bool FileManager::checkSimilarOutputGrid(std::unique_ptr<Grid> &grid) {
    ...
    for (int y = 0; y < grid->getHeight(); y++) {
        ...
        for (int x = 0; x < grid->getWidth(); x++) {
            ...
            if (value != static_cast<int>(grid->getCell(x, y)->getState())) {
                return false;
            }
        }
    }
